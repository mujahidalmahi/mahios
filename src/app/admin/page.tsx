...
export default function AdminOverviewPage() {
  ...
  const quickActions = [
    ...
    { label: '+ Add Status Update', href: '/admin/content/feed', color: 'bg-cyan-600 hover:bg-cyan-500' },
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-5">
        <div>
          <h1 className="text-2xl font-black text-white flex items-center gap-2.5">
            ...
            <span>MahiOS Total Command Center</span>
          </h1>
          <p className="text-xs text-slate-400 mt-1">
            Dynamic administrative suite with 100% total control across all 26 database models and retro operating system apps.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-3.5 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-lg text-xs font-semibold flex items-center gap-2 transition-all cursor-pointer shadow-md"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex items-center gap-3.5 shadow-lg">
          ...
          <div>
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs font-bold text-white uppercase tracking-wider">Supabase Database</span>
            </div>
            <p className="text-[11px] text-slate-400 font-mono mt-0.5">PostgreSQL Engine Active</p>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex items-center gap-3.5 shadow-lg">
          ...
          <div>
            <span className="text-xs font-bold text-white uppercase tracking-wider">Next.js 16 Runtime</span>
            <p className="text-[11px] text-slate-400 font-mono mt-0.5">Turbopack SSR Streaming</p>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex items-center gap-3.5 shadow-lg">
          ...
          <div>
            <span className="text-xs font-bold text-white uppercase tracking-wider">RLS Security Mode</span>
            <p className="text-[11px] text-slate-400 font-mono mt-0.5">Authenticated Admin Policy</p>
          </div>
        </div>
      </div>
      ...
      {/* Entities Metric Grid */}
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider">
            All Content & Studio Entities
          </h2>
          ...
          {statCards.map((card, idx) => {
            ...
            return (
              <Link
                key={idx}
                href={card.href}
                className="bg-slate-900 border border-slate-800 rounded-xl p-4 hover:border-slate-700 transition-all flex flex-col justify-between gap-3 group shadow-lg"
              >
                <div className="flex items-start justify-between">
                  <div className={`p-2 rounded-lg border ${card.bg} ${card.color}`}>
                    <Icon className="w-4 h-4" />
                  </div>
                  <span className="text-xl font-black font-mono text-white group-hover:text-blue-400 transition-colors">
                    {card.count}
                  </span>
                </div>
                <div>
                  ...
                  <p className="text-[10px] text-slate-500 font-mono mt-0.5">Manage Studio →</p>
                </div>
              </Link>
            );
          })}
